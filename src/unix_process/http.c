...
{
    buffer b = allocate_buffer(h, h, allocate(h, 100), 100);
                        ...
                        buffer c)
{
    ...
    // status from t
    buffer d = allocate_buffer(transient, transient, allocate(transient, 1000), 1000);
...
static void reset_parser(http_parser p)
{
    ...
    p->word = allocate_buffer(p->h, p->h, allocate(p->h, 10), 10);
    ...
    for (i = b->start ; i < b->end; i ++) {
        ...
        case STATE_START_LINE:
            ...
            case '\r':
                ...
                p->word = allocate_buffer(p->h, p->h, 0, 0);                
                break;                
            case ' ':
                vector_push(p->start_line, p->word);
                p->word = allocate_buffer(p->h, p->h, 0, 0);
